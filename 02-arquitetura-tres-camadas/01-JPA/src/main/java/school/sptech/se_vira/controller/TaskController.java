...
public class TaskController {
    ...
    public ResponseEntity<List<TaskResponseDto>> listAll() {
        List<TaskResponseDto> tasks = TaskMapper.toResponseDtoList(service.findAll());
    ...
    public ResponseEntity<TaskResponseDto> findById(@PathVariable Integer id) {
        return ResponseEntity.ok(TaskMapper.toResponseDto(service.findById(id)));
    }

    // POST /tasks
    @PostMapping
    public ResponseEntity<TaskResponseDto> create(@RequestBody @Valid TaskRequestDto dto) {
        TaskResponseDto created = TaskMapper.toResponseDto(service.create(TaskMapper.toEntity(dto)));
    ...
    public ResponseEntity<TaskResponseDto> update(
        ...
        return ResponseEntity.ok(TaskMapper.toResponseDto(service.update(id, TaskMapper.toEntity(dto))));
    }
    ...
    public ResponseEntity<List<TaskResponseDto>> findByStatus(@PathVariable TaskStatus status) {
        List<TaskResponseDto> result = TaskMapper.toResponseDtoList(service.findByStatus(status));
    ...
    public ResponseEntity<List<TaskResponseDto>> searchByTitle(@RequestParam String keyword) {
        List<TaskResponseDto> result = TaskMapper.toResponseDtoList(service.findByTitleKeyword(keyword));
        if (result.isEmpty()) return ResponseEntity.noContent().build();
        return ResponseEntity.ok(result);
    }
